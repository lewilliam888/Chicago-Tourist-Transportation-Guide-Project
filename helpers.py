...
def load_l_stops(url):
    ...
    df = df[['stop_id', 'stop_name', 'latitude', 'longitude']]  # Select relevant columns
...
def create_transit_map(landmark_lat, landmark_lon, landmark_name, stop_lat, stop_lon, stop_name, stop_type):
    ...
    folium.Marker(
        ...
        popup=f"<b>{stop_name}</b><br>{stop_type}",
        tooltip=stop_name,
